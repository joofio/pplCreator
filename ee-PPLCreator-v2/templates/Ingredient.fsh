...
* substance.strength.referenceStrength.strengthRatio.denominator =  1 $200000000014#{{row["Pakendi suurus"].split(",")[0]|get_by_regex("[A-Za-z]+")|get_data_dictionary_info(200000000014,"RMS termini id","RMS termini nimi")}} "{{row["Pakendi suurus"].split(",")[0]|get_by_regex("[A-Za-z]+")|get_data_dictionary_info(200000000014,"RMS termini nimi","RMS nimi eesti keeles") }}"
* substance.strength.referenceStrength.substance.concept = $sms#{{ row["Toimeaine"].split(",")[idx]|strip_spaces|get_data_dictionary_info("substance","Concept Code","National Description")  }} "{{ row["Toimeaine"].split(",")[idx]|strip_spaces|get_data_dictionary_info("substance","Description (FSN)","National Description")   }}"
